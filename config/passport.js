...
module.exports = function(passport) {
    ...
    function(req, email, password, done) {
        ...
        baseController.countUser().then(dataUser => {
              ...
              var active_code=bcrypt.hashSync(Math.floor((Math.random() * 99999999) *54), null, null);
                ...
                newUser.save(function(err) {
                  ...
                  /*  var email            = require('../lib/email.js');
                    ...
                    */
                    return done(null, newUser,req.flash('success', '<b>' + req.body.username + '</b> Tạo tài khoản thành công!'));
    ...
    function(req, email, password, done) { // callback with email and password from our form
        ...
        User.findOne({ 'mail' :  email }, function(err, user) {
            ...
            if (!user)
                return done(null, false, req.flash('error', 'Tài khoản hoặc mật khẩu không đúng!')); // req.flash is the way to set flashdata using connect-flash

            
            
            // if the user is found but the password is wrong
            if (!user.validPassword(password))
                return done(null, false, req.flash('error', 'Email và mật khẩu không đúng!')); // create the loginMessage and save it to session as flashdata

            if(user.status === 'inactive')
             return done(null, false, req.flash('error', 'Tài khoản chưa được xác thực!')); // create the loginMessage and save it to session as flashdata
